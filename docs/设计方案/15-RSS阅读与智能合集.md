...
> 版本：0.1
> 日期：2026-08-11
> 状态：待用户评审；UI-023 尚未开始
> 边界：定义前端 UI、交互、概念状态与未来适配边界；本轮不新增前端、Go、依赖、生成文件或 Wails 协议
...
- [ ] 左侧栏“阅读”使用 `Rss`，紧贴“应用”下方，其他菜单相对顺序不变。
- [ ] 页面名称只在全局标题栏显示，内容区没有重复“阅读”标题或第二条页面标题栏。
- [ ] 标准窗口为文章列表 + 阅读窗；合集选择器位于列表区域，没有第四列。
- [ ] 窄窗口可在列表与正文间单页切换，返回后恢复焦点与滚动位置。
- [ ] 打开全局右栏不会形成四列；底部面板可恢复且不压垮主内容。

### 17.2 合集与排序

- [ ] 系统合集只有精选、全部，且不可重命名、删除或重排。
- [ ] 全部包含活跃订阅及保留历史的所有文章。
- [ ] 所有合集按发布时间倒序，AI 不改变排序；缺失发布时间有明确回退标记。
- [ ] 相似或重复报道逐篇保留，不聚合、不折叠、不隐藏。
- [ ] 来源合集必须选择至少一个 Feed，并包含这些来源的全部文章。
- [ ] AI 合集不显示来源选择器，分析全部活跃订阅的新文章。
- [ ] 来源和文章可以同时属于多个合集。
- [ ] 两类自定义合集不可互换；删除合集不删除文章或 Feed。

### 17.3 AI、隐私与反馈

- [ ] AI 合集历史范围包含仅新文章、7/30/90 天、全部，默认 30 天。
- [ ] 创建前可看匹配预览，覆盖无文章、零匹配、部分失败和取消恢复。
- [ ] 精选基线包含可信度、证据丰富度、信息密度、原创性、新鲜度和用户相关性。
- [ ] UI 明确说明质量与相关性是模型判断，不是事实保证。
- [ ] AI 使用阅读全局默认模型，单个 AI 合集可覆盖。
- [ ] 首次启用远程模型前列明发送正文、元数据、用户要求与反馈规则；明确不发送 Feed 凭据。
- [ ] 用户拒绝远程授权时不发送数据，全部和来源合集仍可用。
- [ ] 每篇匹配文章显示入选理由，不展示伪精确分数或思维链。
- [ ] “更符合/不符合”只影响当前合集，形成可查看、可撤销规则。
- [ ] 阅读、收藏、跳过或停留不会暗中学习，反馈不静默改写文字要求。
- [ ] Feed 文章提示注入不能触发工具、副作用或设置修改。

### 17.4 订阅、迁移与阅读

- [ ] 网站 URL 和 Feed URL 都进入候选预览；支持公开 RSS、Atom、JSON Feed 规格。
- [ ] 多候选由用户选择，重复源合并到现有订阅。
- [ ] OPML 导入有预览、重复合并、部分失败和逐项结果。
- [ ] 一个 OPML Feed 可因多个文件夹进入多个来源合集。
- [ ] 标准 OPML 只承载订阅源与来源合集；AI 定义由 Aestival 配置导出承载。
- [ ] 订阅源可重命名、暂停、恢复、刷新、分配合集和取消订阅。
- [ ] 取消订阅使用危险确认，默认保留历史，也可同时删除。
- [ ] 阅读窗只展示 Feed 正文或摘要；摘要有提示与“打开原文”。
- [ ] 支持已读/未读、收藏、批量标记、复制链接、打开原文和 AI 反馈。
- [ ] 收藏是筛选条件，不出现第三个系统合集。
- [ ] 三种默认打开方式与每篇文章临时覆盖均可用。
- [ ] 三种已读策略均可用，默认打开时；未读数字可隐藏。
- [ ] 正文搜索只在阅读页内；全局 Command 只提供阅读和合集导航。

### 17.5 状态、组件与 Mock 诚实性

- [ ] 覆盖无订阅、无文章、验证中、部分导入、失效源、离线和打开失败。
- [ ] 覆盖模型未配置、远程未授权、排队、分析、失败和零匹配。
- [ ] Feed 与 AI 分类状态使用明确状态机，不用冲突 boolean 表达。
- [ ] Mock 数据、未来适配器与页面状态分离，UI 不声称真实发现、刷新、导入或分类成功。
- [ ] 所有菜单有 Lucide 图标，纯图标有 Tooltip/aria-label，危险动作使用 AlertDialog。
- [ ] 主要流程可键盘完成，浮层关闭后焦点返回正确。
- [ ] 页面无 Card 嵌套、视觉卡片嵌套或卡片墙。

### 17.6 后续任务边界

- DOC-006 用户验收前，UI-023 保持待办，不修改前端、Go、依赖或生成文件。
- UI-023 只实现前端 Mock 与明确演示反馈，不接真实抓取、模型、文件或持久化。
