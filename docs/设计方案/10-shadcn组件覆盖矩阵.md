...
| `Resizable` | 主工作区、右侧栏、底部面板、设置、应用编辑器 | 横向/纵向分割、最小阈值、恢复默认 |
...
| `Toggle Group` | 代理/聊天收起态、列表/网格、日历视图、搜索选项 | 2–7 个互斥或多选选项 |
...
| `Date Picker` | 统计范围、任务单次日期 | 日期或范围选择 |
| `Calendar` | 任务日历、单次触发 | 月/周日期交互和计划点 |
...
| `Alert Dialog` | 删除、断开、清空、绕过审批、替换文件 | 明确不可逆或高风险后果 |
| `Sheet` | 宽屏详情、右侧检查器、窄屏侧栏 | 从边缘进入的详情/工具区 |
