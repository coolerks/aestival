...
- 源视觉真值：`/var/folders/jz/v6ymlznd733_t1sqjwrc4zdm0000gn/T/codex-clipboard-93fadad9-f0cd-4ec0-9145-b085946549b7.png`。
- 源图像素：1656 × 1086，deviceScaleFactor 约为 2；将标题栏聚焦区域按 0.5× 归一化到 CSS 像素。
- 实现截图：`docs/协作同步/design-qa-title-placement-1280x720.png`，CSS 视口 1280 × 720，截图像素 1280 × 720；浏览器报告 devicePixelRatio 2，但截图 API 输出 CSS 像素。
- 聚焦合并对照：`docs/协作同步/design-qa-title-placement-comparison.png`，600 × 250；上方为归一化源图，下方为实现。
- 状态：浅色、新建任务、Sidebar 展开，右侧栏和底部面板关闭。

### Findings、修复与复核

- [P1] 先前名称仍出现在左侧窗口控件段，未落到标注指向的主标题栏空白区。已将 `AppTitlebar` 改为明确的左右两段布局：左段只包含 72px 原生窗口安全区和侧栏按钮，右段渲染唯一标题与右侧动作。
- [P2] 绝对定位标题与标题栏按钮层级分离，容易在构建或响应式状态下产生视觉归属歧义。已移除标题绝对定位，标题直接成为主标题段的布局子元素。
- [P2] 侧栏触发器存在 button 嵌套，可能干扰标题栏点击/拖动命中。已改为一个 Button 直接作为 TooltipTrigger。
- 修复后量测：展开态侧栏边界 x=256px、标题 x=272px，偏移 16px；收起态侧栏占位 0px、标题 x=140px；恢复后重新为 x=272px。
- 修复后可访问树中 banner 只有一个“新建任务”标题；新产生的标题栏错误为 0。控制台仍有来自任务输入 Popover 的既有 Base UI 语义警告，不属于本轮标题位置变更。

### 必查视觉面

- 字体与排版：继续使用 Geist/系统中文回退，标题保持 14px medium、20px 行高，垂直中心 y=16.5px。
- 间距与布局：标题与侧栏边界固定相距 16px；53px 标题栏高度、256px 侧栏宽度和右侧动作间距不变。
- 颜色与 token：继续使用 `bg-background`、`border-border` 和语义前景色，没有新增硬编码颜色或视觉材质。
- 图像与图标：未改动品牌 SVG；侧栏开关继续使用 Lucide `PanelLeftIcon`，不存在占位或手绘图标。
- 文案与内容：标题仍由当前页面/会话状态提供，只改变唯一展示位置，没有新增重复页面标题。

### 比较历史与结论

1. 首次标注对照发现 P1/P2：标题在左段、依赖绝对定位、侧栏按钮嵌套。
2. 改为左右两段结构并消除嵌套按钮后重新截图；聚焦对照和浏览器量测均证明标题位于标注目标区域。
3. 未发现剩余与本轮标题位置相关的 P0/P1/P2 差异。
