...
## UI-021 项目看板与甘特图设计 QA

### 对照与实现范围

- 视觉参考：`/var/folders/jz/v6ymlznd733_t1sqjwrc4zdm0000gn/T/codex-clipboard-9ab141c9-d1ad-4057-9067-9861ed2c78eb.png`。
- 参考图采用宽松的四列看板；Aestival 按已确认需求展示固定五列，并沿用现有 Codex 风格的紧凑灰阶、细边框、低装饰和单层任务 Card。
- 主内容区与右侧面板使用同一工作项数据，分别保存日期范围、看板/甘特视图与选中状态；右侧窄容器自动退化为单状态列。
- 甘特图与看板均由表面最外层承担主要滚动，列和任务列表没有再建立互相竞争的纵向滚动区。

### 浏览器回归证据

- 在 `960×640`、`1280×800`、`1440×900`、`1920×1080` 四个视口检查主看板，并检查主看板与右侧面板同时打开状态；页面根节点 `scrollWidth` 均未超过视口宽度。
- 标题栏入口位于搜索按钮之后，可访问名称为“打开任务项目看板”；打开后全局标题唯一显示为“任务 · 看板”，内容区没有重复主标题。
- 五列、空列放置区、任务详情、日期预设、自定义范围、未排期/已作废筛选、看板/甘特切换均能显示；右侧面板在窄容器中使用状态切换器与单列任务。
- 主看板保持看板视图时，右侧面板可独立切换到甘特图；两者视图状态不互相覆盖。
- 从“任务”切换到“Aestival”后标题和工作项同时切换，未出现跨项目数据串联；执行一次 Mock AI 规划后，新增工作项同时出现在主看板与右侧面板。
- 浏览器控制台没有新增 error；任务卡片无 Card 嵌套，页面未形成卡片墙。

### 交互与边界结论

- 五状态使用统一命令与权限策略；AI 完成、重新打开、作废和恢复均被拒绝，人类只能从“待验收”确认进入“已完成”。
- 看板拖放由 DnD Kit 提供鼠标与键盘传感器；人类普通移动使用乐观更新与撤销 Toast，进入完成态必须经过确认。
- 当前数据、审计和 AI 规划均为内存 Mock；刷新会重置，甘特图首版只读，不代表真实 Wails 持久化或 AI 执行成功。

### UI-021 结论

未发现阻断验收的 P0/P1/P2 视觉或交互问题。实现满足五列看板、只读甘特、主/右双表面同步、项目隔离、权限边界、窄栏退化和单一主要滚动画布要求。
