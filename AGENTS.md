...
- 当前页面、会话或文件的名称只允许显示在全局标题栏的主内容段，并与主内容左缘留白对齐；展开侧栏时，侧栏材质段只保留原生窗口控件和侧栏开关，不得放置当前名称。主内容区不得重复同名主标题，也不得再创建带背景或底边的第二条页面标题栏。内容区可以使用不重复页面名称的章节标题、Breadcrumb、说明、状态和操作工具栏。
