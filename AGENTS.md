...
- 左侧栏“项目”标题行右侧使用 `SidebarGroupAction` 提供添加入口；鼠标 Hover 时显示，同时必须在键盘聚焦和无 Hover 设备上可见，纯图标入口提供 `aria-label`、Tooltip 与快捷键。
- 用户创建的工作区分为不可转换的“项目”和“笔记”两类；两类都至少选择一个根目录、允许多个根目录并指定唯一默认根目录，禁止重复路径和父子目录重复纳入。固定“任务”项目不受根目录校验约束。
- 左侧栏“阅读”使用 Lucide `Rss`，固定在“应用”正下方；阅读页以文章列表和阅读窗组成主内容，合集选择器并入列表区域，不得形成第四列。
- 主内容的“聊天”页签始终位于第一项且不可关闭。
- 右侧栏和底部面板支持文件、终端、内容搜索、日志、会话调试。
- 普通项目底部面板首次展开默认打开终端；笔记项目首次展开默认打开搜索。
- 普通项目继续使用现有 Monaco、终端、日志、会话调试和项目看板；笔记项目使用 Vditor 的源码、并排、即时和预览四模式，右侧栏只允许文件、搜索、大纲、反向链接、标签与属性、局部图谱，底部只允许搜索与反向链接。
- 项目切换必须按项目隔离页签、编辑组、面板、尺寸和笔记视图快照；两种项目类型的面板状态不得互相恢复或污染。
...
- 顶层功能：代理/聊天、知识库、全局搜索、应用、阅读、能力、任务、设置。
...
- 阅读的系统合集只允许“精选”和“全部”；全部保持完整时间倒序，精选和 AI 合集只筛选、不重排，相似或重复报道逐篇保留。
- 自定义阅读合集分为不可互换的订阅源合集与 AI 合集；前者包含用户所选 Feed 的全部文章，后者不选择来源并按文字要求分析所有活跃订阅的新文章，来源和文章均可属于多个合集。
- 阅读 AI 只允许显式反馈形成当前合集内可查看、可撤销的规则，不得根据已读、收藏、跳过或停留暗中学习，也不得静默改写用户文字要求。
- 远程模型首次用于阅读前必须说明会发送文章正文/摘要、元数据、用户要求与当前合集反馈规则，并明确不发送 Feed 凭据；未授权时不得发送。
- 首版阅读只展示 Feed 提供的正文或摘要，不实现私有鉴权 Feed、网页全文提取、原网页嵌入、播客专项体验或相似报道聚合。
- 未来 Feed 抓取器必须防范不受信任 URL、重定向、内网/云元数据访问、超大或压缩炸弹响应、恶意 XML/JSON 与不安全 HTML；实现前另行完成后端与安全评审。
- 代码和常见二进制预览规则以 `docs/设计方案/09-工作区面板与文件预览.md` 为准。
- 笔记项目只有 Markdown 可编辑；图片、PDF、音视频使用安全预览，代码和纯文本只读。知识图谱边只允许来自显式 Markdown 链接和 `[[WikiLink]]`，不得生成 AI 推测关系。
- 聊天模式在任何项目类型下都禁止工具和副作用；笔记项目的代理模式只允许已配置根目录内任意层级、任意文件类型的受审批文件操作、搜索和内容/图片素材生成，禁止越界、Shell、构建、调试、文件执行和项目看板动作。
...
- 当前只实现 UI 时，可以提供明确标记的 mock 反馈，但不得伪造真实网络连接、计费、文件写入、终端执行、MCP/Skill 安装、数据库同步、Feed 发现/刷新/导入或模型分类成功。
- 项目与笔记 UI Mock 可以使用内存项目、真实原生目录选择和明确标记的保存/审批样例；不得声称完成项目持久化、目录扫描、文件读写、链接索引、知识图谱计算或图片生成。
