...
function TaskTableRow({ task }: { task: MockTask }) {
  ...
  return <TableRow tabIndex={0} onDoubleClick={() => openDetails(task.id)} onKeyDown={(event) => { if (event.target !== event.currentTarget) return; if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openDetails(task.id) } }}><TableCell><div className="flex items-center gap-2"><span className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-muted"><Icon className="size-4" /></span><div className="min-w-36"><div className="flex items-center gap-1.5 font-medium">{task.name}{task.risk ? <Badge variant="destructive">高风险</Badge> : null}</div><div className="max-w-56 truncate text-xs text-muted-foreground">{task.description}</div></div></div></TableCell><TableCell><div>{taskTriggerLabels[task.trigger]}</div><div className="text-xs text-muted-foreground">{task.target}</div></TableCell><TableCell><div>{task.schedule}</div><div className="font-mono text-xs text-muted-foreground">{task.cron}</div></TableCell><TableCell>{task.nextRun ? <><div>{task.nextRun}</div><div className="text-xs text-muted-foreground">{task.nextRunRelative}</div></> : <span className="text-muted-foreground">已暂停</span>}</TableCell><TableCell><Badge variant={resultVariant(task.lastResult)}><ResultIcon />{taskResultLabels[task.lastResult]}</Badge></TableCell><TableCell><Switch checked={task.enabled} onCheckedChange={() => toggleTask(task.id)} aria-label={`${task.enabled ? "暂停" : "启用"}${task.name}`} /></TableCell><TableCell><div className="flex justify-end gap-1"><Tooltip><TooltipTrigger render={<Button variant="ghost" size="icon-sm" aria-label={`立即运行${task.name}`} onClick={() => toast.info("已加入前端 Mock 队列，未调用调度器")} />}><PlayIcon /></TooltipTrigger><TooltipContent>立即运行</TooltipContent></Tooltip><TaskMenu task={task} /></div></TableCell></TableRow>
}
